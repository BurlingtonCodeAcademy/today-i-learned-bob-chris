...
class FactStore {
  ...
  //get all posts written in the last seven days
  async thisWeek() {
    let date = new Date()
    let collection = await this.collection()
    let weekEntries = collection.find({ "when": { $gte: (new Date(date.getTime() - (7 * 24 * 60 * 60 * 1000))) } })

    return weekEntries.sort([['when', -1]])
  }
  ...
  */
  ...
  async comment(title/*this should be the ID instead*/, name, text) {
    let collection = await this.collection();
    let cursor = collection.find({ "title": title });
    let comment = {
      name: name,
      body: text
    };
    ...
    cursor.forEach((til) => {
      let postComments = til.comments
      postComments.push(comment);
      console.log("comments array: " + postComments)
      collection.updateOne({ title: title }, { $set: { comments: postComments } })
    });
  }
